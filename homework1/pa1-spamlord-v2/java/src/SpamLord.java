...
public class SpamLord {
    ...
    */
    ...
    private Pattern emailPattern1 = Pattern.compile("([\\w\\.-]+)[\\s]*@[\\s]*([\\w\\.-]+)\\.(edu|EDU|com|COM)");
    private Pattern emailPattern2 = Pattern.compile("obfuscate\\(\'([\\w\\.]+)\',\'([\\w\\.]+)\'\\)");
    private Pattern phonePattern1 = Pattern.compile("[\\(]?[\\s]*([\\d]{3})[\\s]*[\\)\\-\\s][\\s]*([\\d]{3})[-\\s]([\\d]{4})");
    ...
    public List<Contact> processFile(String fileName, BufferedReader input) {
        ...
        try {
            for (String line = input.readLine(); line != null; line = input.readLine()) {
                ...
                line = line.replace("(followed by &ldquo;", "");
                line = line.replace("(followed by \"", "");
                line = line.replace("&#x40;", "@");
                line = line.replace(";", ".");
                line = line.replace("cs stanford edu", "cs.stanford.edu");



                line = line.replaceAll("[\\s]*(?:\\s|\\()(?:DOT|dot|DOM|dom|dt)(?:\\s|\\))[\\s]*", ".");
                line = line.replaceAll("[Ss]erver[\\s]+at", "###");
                line = line.replaceAll("[\\s]*(?:\\s|\\()(?:at|AT|WHERE)(?:\\s|\\))[\\s]*", "@");
                m = emailPattern1.matcher(line);
                while (m.find()) {
                    email = m.group(1).replace("-", "") + "@" + m.group(2).replace("-", "") + "." + m.group(3).replace("-", "");
